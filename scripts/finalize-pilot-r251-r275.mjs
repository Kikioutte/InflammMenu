...
const catalogue = JSON.parse(await readFile("research/pilot-r251-r275.nutrition.json", "utf8"));
const reviewedAt = "2026-08-05";
const excludedIds = new Set(["r252", "r273", "r275"]);
const specialCautions = {
  r252: "Contient gluten et lait. Le halloumi est très salé : cette recette reste hors du planificateur et aucun sel supplémentaire ne doit être ajouté.",
  r264: "Les pois chiches germés doivent être bouillis complètement; ne jamais les consommer crus et respecter la chaîne du froid.",
  r269: "Citron confit et olives peuvent apporter davantage de sodium que le calcul; rincer et ne pas ajouter de sel.",
  r270: "Contient du lupin, avec possibilité d'allergie croisée chez certaines personnes allergiques à l'arachide; le balsamique peut contenir des sulfites.",
};
...
for (const recipe of catalogue.recipes) {
  ...
  const score = 7.3 + Math.min(1.1, nutrition.fibres_g / 17) + Math.min(0.35, nutrition.proteines_g / 55) - Math.min(0.5, nutrition.sodium_mg / 1400) - Math.min(0.45, nutrition.acides_gras_satures_g / 16);
  recipe.score_anti_inflammatoire = Math.max(6.7, Math.min(8.8, Number(score.toFixed(1))));
  recipe.score_note = `Cette salade associe végétaux, fibres et une source de protéines dans un profil alimentaire global; les fromages et produits salés sont pris en compte dans l'appréciation. Cet indice éditorial décrit le profil de la recette; il ne mesure ni ne prouve aucun effet médical.`;
...
catalogue.meta = {
  ...
  nutrition_notice: "Valeurs calculées à partir de Ciqual 2025; les proxies d'ingrédients du monde et valeurs techniques sont signalés recette par recette.",
...
await writeFile("research/pilot-r251-r275.final.json", `${JSON.stringify(catalogue, null, 2)}\n`);
console.log(`${catalogue.recipes.length} recettes r251-r275 finalisées, ${catalogue.recipes.filter((recipe) => recipe.app.planner.eligible).length} éligibles.`);
