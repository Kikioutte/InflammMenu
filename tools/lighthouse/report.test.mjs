...
function report() {
  return {
    ...
    audits: {
      ...Object.fromEntries(METRICS.map(key => [key, { numericValue: 100 }])),
      'network-requests': { scoreDisplayMode: 'informative', details: { items: [] } },
      'errors-in-console': { scoreDisplayMode: 'binary', details: { items: [] } },
    },
...
test('un contrôle réseau ou console absent, échoué ou ignoré ne prouve jamais une absence d’erreur', () => {
  for (const key of ['network-requests', 'errors-in-console']) {
    for (const change of [r => { delete r.audits[key]; }, r => { r.audits[key].scoreDisplayMode = 'error'; }, r => { r.audits[key].scoreDisplayMode = 'skipped'; }, r => { r.audits[key].errorMessage = 'Failed'; }, r => { delete r.audits[key].details; }]) {
      const value = report(); change(value); assert.throws(() => readMeasurement(value), /audit/);
    }
  }
});
