...
- Direction visuelle fusion validée le 7 septembre 2026 : fond ivoire, titres éditoriaux olive, photographies existantes généreuses, cartes aérées et navigation explicite. Le constructeur « À votre table » conserve une recherche par catégorie Entrée–Plat–Dessert et un récapitulatif visible, avec recalcul de la compatibilité globale. Les données, photographies et règles existantes restent inchangées. Toute intégration passe par une branche et une pull request vérifiée.
...
- Décision du 8 septembre 2026 : un dessert d’appoint de la collection d’associations peut être ajouté manuellement comme complément d’une entrée et d’un plat vérifiés ensemble. Il ne devient pas une recette tirée seule par le générateur. Les exclusions éditoriales, les doublons et les critères du profil restent appliqués.
- Les cartes d’un repas composé affichent les noms de l’entrée, du plat et du dessert, disponibles aussi hors connexion. Modifier un repas planifié conserve son créneau et ses portions, revérifie le repas complet et actualise les courses ; si le créneau a changé entre-temps, demander de rouvrir le repas.
- « Mes repas » permet de nommer une composition, de la rechercher par son nom ou celui de ses recettes et d’annuler sa dernière suppression. Le nom fait partie de la sauvegarde exportable.
- La recherche tolère une petite faute dans les mots longs et plusieurs mots dans un ordre différent. Cette souplesse reste limitée à la recherche : jamais aux identifiants d’ingrédients, allergies, exclusions ou associations.
- Les courses peuvent recevoir des recettes et des articles libres sans créer de semaine. Les recettes ajoutées conservent un instantané et leurs portions, se cumulent aux achats planifiés via les correspondances d’achat existantes, et n’entrent jamais dans le générateur. Une liste de courses ne constitue pas un repas compatible. Les exclusions éditoriales restent appliquées et les ajouts structurés respectent le régime, les allergies et les ingrédients exclus du profil.
- Les collections personnelles classent des références de recettes indépendamment des favoris. Les collections et courses libres sont incluses dans la sauvegarde, la restauration et la synchronisation locale entre onglets.
- `app.planner.eligible: false` est une décision de relecture par recette (sodium élevé, interaction connue, brouillon, ou catégorie d'appoint). Elle se rend visible via `plannerAvailabilityFor` — recette d'appoint ou exclusion éditoriale — et ne se contourne jamais, sauf pour le dessert complémentaire explicitement autorisé ci-dessus.
...
- « Que cuisiner ce soir ? » répartit doucement les soupes, salades et bowls dans chaque groupe de six afin que les suggestions restent variées quand des alternatives existent. Cette préférence ne retire jamais une recette compatible et reste moins forte que les bonus du garde-manger et des favoris.
- Le générateur évite autant que possible de servir deux soupes, deux salades ou deux bowls le même jour. Cette variété de forme reste une préférence souple : elle ne contourne jamais les filtres stricts, les objectifs hebdomadaires, un repas conservé ni un budget qui ne peut être tenu autrement. Le rééquilibrage budgétaire épuise d’abord les économies qui n’aggravent pas la monotonie.
...
- `src/Prototype.tsx` is the stable entry point. Build app-specific screens in `src/screens/`, shared UI in `src/components/`, and orchestration in `src/app/AppShell.tsx`; keep styles in `src/prototype.css`. See `docs/ui-architecture.md` for ownership and extraction invariants.
- Keep the single recipe registry in `src/app/recipe-registry.ts` and the single application store in `src/app/app-state-store.ts`. Import their live bindings; do not copy mutable registry values into module-level snapshots. Screens must not import `Prototype` or `AppShell`.
...
- For a layered app shell—such as a persistent composer, independently presented sheet, pushed/peek sidebar, or app-wide transition—compose directly in `src/app/AppShell.tsx` rather than forcing it through `FlowStack`. Keep app-owned fixed chrome as sibling layers outside `MobileScroll`.
